...
export async function POST(request: NextRequest) {
  try {
    console.log('=== Word文档生成API请求开始 ===');
    
    const body = await request.json();
    console.log('前端请求体:', body);
    
    // 转发请求到Python后端
    const response = await fetch(`${PYTHON_BACKEND_URL}/financial-report/generate-word-only`, {
      method: 'POST',
      headers: {
    ...
    console.error('Word文档生成API错误:', error);
    return NextResponse.json(
      { 
        success: false,
        error: `Word文档生成失败: ${error instanceof Error ? error.message : '未知错误'}`
      },
      { status: 500 }
    );
  }
